...
if($_SERVER['REQUEST_METHOD']=="GET") {
    ...
    while($view = DB::fetch($query)) {
        ...
        if($Model==0){
            ...
            $message=tool::discuzcode($message, 0,True,True);
            $msg = tool::strip_ubb($msg);
            $message=tool::deletetime($message,$Model);
        }elseif ($Model==1){
            preg_match_all('/\[attach\](.*?)\[\/attach\]/i', $message,  $attach);
            $attcount=count(explode('[attach]',$message));
            if ($attcount>=2){//取图片
                ...
                $attquery = DB::query($sql);
                    if($attrequ = DB::fetch($attquery)) {
                        $atttableid = $attrequ['tableid'];
                        $attaid=$attrequ['aid'];
                        $attimg= UC_API."/"."data/attachment/forum/".$attrequ['attachment'];
                        $attachm= tool::deletetime("[attach]$attaid [/attach]");
                        if (intval($attrequ['isimage'])==1){
                              $attachimg= tool::deletetime("[img=0,0]$attimg [/img]");
                        }else{
                            $filename = $attrequ['filename']; 
            ...
            $message = tool::discuzcode($message,1,False,False);
            $msg = tool::strip_ubb($msg);
            $message = tool::strip_ubb($message);
        }
        $flow = $lou;
