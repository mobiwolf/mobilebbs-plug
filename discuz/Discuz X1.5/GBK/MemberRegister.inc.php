...
if($_SERVER['REQUEST_METHOD']=="POST") {
	$hname = trim(iconv("utf-8","gb2312",urldecode(base64_decode($_SERVER['HTTP_USERNAME']))));
    $hpwd = trim(base64_decode($_SERVER['HTTP_PASSWORD']));
    ...
    if (empty($hname)){
        ...
        $ecode = base64_encode("4");
        echo "{\"error\":\"$ecode\"}";
    }else if(strlen($hname)<3) {
        $ecode = base64_encode("1");
        echo "{\"error\":\"$ecode\"}";
    }else if(strlen($hname)>15){
        $ecode = base64_encode("1");
        echo "{\"error\":\"$ecode\"}";
    }
    ...
    $_G['gp_username'] = $hname;
    $_G['gp_password'] = $hpwd;
    $_G['gp_password2'] = $hpwd;
    $_G['gp_email'] = $hmail;
    ...
    switch($_G['setting']['regverify']) {
        ...
        echo "{\"error\":\"$ecode\",\"uid\":\"$uid\"}";
    } 
}
else {
    $ecode = base64_encode("-11");
    echo "{\"error\":\"$ecode\"}";
}
