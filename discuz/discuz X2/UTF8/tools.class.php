...
class tool{
  ...
  public static function delmsg($str,$model=0)
  {
    ...
    //对[....] ..数据进行处理
    public static function discuzcode($message,$model ,$smileyoff, $bbcodeoff, $htmlon = 0, $allowsmilies = 1, $allowbbcode = 1, $allowimgcode = 1, $allowhtml = 0, $jammer = 0, $parsetype = '0', $authorid = '0', $allowmediacode = '0', $pid = 0, $lazyload = 0) {
    global $_G;
    ...
    if(!$bbcodeoff && $allowbbcode) {
        ...
        $message = str_replace(array(
            ...
            '</font>', '</font>', '</font>', '</font>', '</p>', '<strong>', '</strong>', '<strike>', '</strike>', '<hr />', '</p>', '<i >', '<i>',
            '</i>', '<u>', '</u>', '<ul>', '<ul type="1" >', '<ul type="a" class="">',
            '<ul type="A" class="litype_3">', '<li>', '<li>', '</ul>', '', '', '</span>'
            ), preg_replace(array(
            "/\[color=([#\w]+?)\]/i",
            "/\[color=(rgb\([\d\s,]+?\))\]/i",
            "/\[backcolor=([#\w]+?)\]/i",
        ...
        if($parsetype != 1) {
            if(strpos($msglower, '[/quote]') !== FALSE) {
                $message = preg_replace("/\s?\[quote\][\n\r]*(.+?)[\n\r]*\[\/quote\]\s?/is", tool::tpl_quote(), $message);
            }
        ...
        if(strpos($msglower, '[/flash]') !== FALSE) {
            $message = preg_replace("/\[flash(=(\d+),(\d+))?\]\s*([^\[\<\r\n]+?)\s*\[\/flash\]/ies", $allowmediacode ? "parseflash('\\2', '\\3', '\\4');" : "bbcodeurl('\\4', '", $message);//<a href=\"{url}\" target=\"_blank\">{url}</a>')
        }
    ...
    if(!$bbcodeoff) {
        ...
        if(strpos($msglower, '[/img]') !== FALSE) {
            $message = preg_replace(array(
                ...
                (!defined('IN_MOBILE') ? "bbcodeurl('', '')" : "bbcodeurl('', '')"),
                (!defined('IN_MOBILE') ? "bbcodeurl('', '')" : "bbcodeurl('', '')"),
            ), $message);
        } 
        }else{
        if(strpos($msglower, '[/img]') !== FALSE) {
            $message = preg_replace(array(
                ...
                (!defined('IN_MOBILE') ? "bbcodeurl('\\1', '<img src=\"{url}\" target=\"_blank\"></img>')" : "bbcodeurl('\\1', '<img src=\"{url}\" ></img>')"),
                (!defined('IN_MOBILE') ? "bbcodeurl('\\3', '<img src=\"{url}\" target=\"_blank\"></img>')" : "bbcodeurl('\\3', '<img src=\"{url}\" target=\"_blank\"></img>')"),
            ), $message);
        }
        }
    }
