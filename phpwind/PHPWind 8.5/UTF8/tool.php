...
function convert($message,$model,$allow,$type="post"){
	...
	$message = str_replace("p_w_upload",$db_attachname,$message);//此夜位置不可调换
	$message = str_replace("p_w_picpath",$db_picpath,$message);//此夜位置不可调换
	...
	if ($allow['pic']) {
		...
		$message = preg_replace("/\[img\]([^\<\r\n\"']+?)\[\/img\]/eis","nopic('\\1',$model)",$message,$db_cvtimes);
	...
	if ($type == 'post') {
		...
		strpos($message,'[s:') !== false && $message = showface($message,$model);
	}
...
function nopic($url,$model) {
	global $code_num,$code_htm,$imgpath,$stylepath;
	$code_num++;
	$cotenthttp = explode("http:",$url);
	if ($model==0){}
	elseif ($model==1){
	if (count($cotenthttp)>1){
		$strimg ="<img src=\"".$url."\" align=\"absbottom\" border=\"0\" /> ";
		
	}else{
		$strimg ="<img src=\"".UC_API."/".$url."\" align=\"absbottom\" border=\"0\" /> ";
	}
	}
	return $strimg;
}
...
function showface($message,$model) {
	...
	$message = preg_replace("/\[s:(.+?)\]/eis","postcache('\\1',$model)",$message,$db_cvtimes);
	return $message;
}
function postcache($key,$model) {
	...
	if ($face[$key][2]) {
		...
		return "<img src=\"".UC_API."/$imgpath/post/smile/{$face[$key][0]}\" />";
	}
	}
}
...
function table($tab){
    define('D_P',dirname(dirname(dirname(__FILE__))).'/');
    require_once D_P.'data/sql_config.php';
    return "$pre"."$tab"; 
}
function getstatebyuid($uid){
  global $db;
  $retAry = array("state"=>base64_encode('0'),
                  "sdate"=>"",
                  "edate"=>""
                  );
  if (intval($uid)>0){
  $fsql = "select groupid from pw_members where uid ='$uid'";
  $con = $db->get_value($fsql);
  if ($con == 6){
     $lsql = "select type,startdate,days from pw_banuser where uid='$uid' AND fid='0'";
     $query = $db->query($lsql);
     while ($rt = $db->fetch_array($query)) {
           $type = $rt['type'];
           $retAry['state'] = base64_encode($type); 
           if (intval($type)==1){
              $sdate = date("Y-m-d H:i:s",$rt['startdate']);
              $retAry['sdate'] = base64_encode($sdate);
              $day = $rt['days'];
              $retAry['edate'] = base64_encode(date("Y-m-d H:i:s",strtotime("$sdate + $day days"))); 
           }
     }
  }
  }
...
class attachShow {
	 ...
	 * @param string $uploadset 附件展示设置(是否显示图片???
	 * @param bool $isConfineGuset 是否限制游客浏??a
	 * @param string $mode 应用场景(帖子、活动�?日志???
	 */
	...
	/**
	 * 从数据库获取指定id的附件列表，初??d化类库信???
	 * @param int $tid 帖子tid
	 * @param array $pids 回??Dpid集合
	 */
	...
	/**
	 * 解析内容中的附件信息
	 * @param int $pid 回??Dpid
	 * @param string $content 帖子内容
	 * @param bool $isSelf 是否是本帖作???
