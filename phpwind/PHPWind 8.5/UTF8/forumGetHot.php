...
if($_SERVER['REQUEST_METHOD']=="GET") {
    ...
    $tabPre = $db->dbpre;
    $tquery = $db->query("select * from ".$tabPre."threads left join ".$tabPre."tmsgs using(tid) where hits >1 ".
              "and fid!=0  order by postdate desc LIMIT 0,$Size");
	$zjson['list']=array();
	while ($forums = $db->fetch_array($tquery)) {
		...
		$platearray = array("tid"           => base64_encode($tid),
                            ...
                            "subject"       => base64_encode(urldecode($subject)),
