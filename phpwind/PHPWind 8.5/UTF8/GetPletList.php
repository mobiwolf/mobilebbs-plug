...
if($_SERVER['REQUEST_METHOD']=="GET") {
    ...
    $tabPre = $db->dbpre;
    $query  = $db->query("SELECT *  FROM ".$tabPre."forums f LEFT JOIN pw_forumdata fd USING(fid) WHERE f.ifcms!=2  $sqlwhere  order by f.vieworder");
...
function arrayResult(&$forum)
{
    ...
    $arr_forum = array( "fid"			=> base64_encode($forum['fid']), 
                        ...
                        "name"			=> base64_encode($forum['name']), 
                        //"threads"		=> $forum['topic'],
                        "posts"			=> base64_encode($forum['article']),
